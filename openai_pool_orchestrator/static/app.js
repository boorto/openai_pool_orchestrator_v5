...
async function batchSync() {
  ...
  btn.disabled = true;
  btn.textContent = '补传中...';
  showToast('批量补传开始', 'info');
  try {
    const payload = JSON.stringify({ filenames: [] });
    const summaries = [];
    let successEndpoints = 0;
    let hasFailure = false;

    const cpaResp = await fetch('/api/cpa-sync-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: payload,
    });
    const cpaData = await cpaResp.json();
    if (cpaResp.ok) {
      successEndpoints += 1;
      summaries.push(`CPA: 共 ${cpaData.total}，成功 ${cpaData.ok}，跳过 ${cpaData.skipped}，失败 ${cpaData.fail}`);
      if ((cpaData.fail || 0) > 0) hasFailure = true;
    } else {
      summaries.push(`CPA: 跳过（${cpaData.detail || '未配置或不可用'}）`);
    }

    const sub2apiResp = await fetch('/api/sync-batch', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: payload,
    });
    const sub2apiData = await sub2apiResp.json();
    if (sub2apiResp.ok) {
      successEndpoints += 1;
      summaries.push(`Sub2Api: 共 ${sub2apiData.total}，成功 ${sub2apiData.ok}，跳过 ${sub2apiData.skipped}，失败 ${sub2apiData.fail}`);
      if ((sub2apiData.fail || 0) > 0) hasFailure = true;
    } else {
      summaries.push(`Sub2Api: 跳过（${sub2apiData.detail || '未配置或不可用'}）`);
    }

    if (successEndpoints === 0) {
      showToast('批量补传失败：CPA / Sub2Api 均不可用', 'error');
      return;
    }
    showToast(summaries.join(' | '), hasFailure ? 'info' : 'success');
    loadTokens();
  } catch (e) {
    showToast('批量补传失败: ' + e.message, 'error');
  } finally {
    btn.disabled = false;
    btn.textContent = '批量补传';
  }
}
